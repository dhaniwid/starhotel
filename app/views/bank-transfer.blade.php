<section class="contact-details" id="contactform">
	<div class="form-group">
		<div class="sectioning section-banktransfer">
			<div class="price">
			    <h2 class="lined-heading"><span>Payment Detail</span></h2>
			    <div class="col-lg-12">
				    <p style="text-align: center; font-weight: bold;">Total amount to be paid {{$reservation->roomprice_rate}}</p>
				    <div class="clear"></div>
				</div>
				<div class='col-lg-12 form-group' style="text-align: center">
	                <div class="secure-text">100% Secured Transaction</div>
	                <button type="submit" id="process" class="btn  btn-lg btn-primary">Process Payment</button>
	            </div>
		        <div class="col-lg-12">
		            <div class="ket-title">Silahkan ikuti langkah ini untuk menyelesaikan pembayaran Anda</div>
		            <ul>
		                <li>Melakukan pembayaran selambat-lambatnya 1 jam setelah pemesanan Anda.</li>
		                <li>Dan silahkan kirim bukti pembayaran melalui email: cs@amarterravilla.com atau fax +62 361 776 300.</li>
		                <li>Transaksi Anda akan dibatalkan (kadaluarsa) jika Anda tidak membayar dalam batas waktu yang ditentukan.</li>
		            </ul>
		        </div>
			</div>
		</div>
	</div>
</section>