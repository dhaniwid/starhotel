...
var showRegisterFormAjaxErrors = function(errors)
{
    ...
    for(var errorType in errors)
    {
        ...
        {
            $('[name="'+errorType+'"]').closest('.form-group').append('<div class="alert alert-danger alert-dismissable">\n\
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'
                +errors[errorType][i]+'</div>').hide().fadeIn(900);
        }
    }
};

var showRegisterFormAjaxSuccess = function(errors)
{
    $('.status-message').remove();
    $('.label-danger').remove();
    for(var errorType in errors)
    {
        for(var i in errors[errorType])
        {
            $('[name="'+errorType+'"]').closest('.form-group').append('<span class="label label-success error-'+errorType+'">'+errors[errorType][i]+'</span>');
        }
    }
};
